  ...
  return (
    <div className='fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-36 py-5'>
      ...
      <div className='flex item-center gap-8'>
          <SearchIcon  className='max-md:hidden mt-3 w-6 h-6 cursor-pointer'/>
          {
            !user ? (
              ...
              <UserButton>
                <UserButton.MenuItems>
                  <UserButton.Action label='My Bookings' labelIcon={<TicketPlus width={15}></TicketPlus>} onClick={()=>navigate('/my-bookings')}></UserButton.Action>
                </UserButton.MenuItems>
              </UserButton>
            )
          }
      ...
      <MenuIcon className='max-md:ml-4 md:hidden w-8 h-8 curson-pointer' onClick={()=>setIsOpen(!isOpen)} />
    </div>
  )
}
